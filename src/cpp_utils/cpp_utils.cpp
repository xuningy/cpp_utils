...
template void cpp_utils::print_matrix<
    Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(
    const Eigen::MatrixBase<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic,
                                          Eigen::RowMajor>> &mat,
    const std::string &func, const std::string &file, const int line);
template void cpp_utils::print_matrix<
...
template void cpp_utils::print_matrix<
    ...
    const Eigen::MatrixBase<Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic,
                                          Eigen::RowMajor>> &mat,
    const std::string &func, const std::string &file, const int line);

template <typename D>
void cpp_utils::print_matrix(const Eigen::MatrixBase<D> &mat,
                             const std::string &func, const std::string &file,
                             const int line) {
  /**
   * @brief:  Print out a matrix, the function, the file and the line number
   * it's called from
   *
   * This method takes as input an Eigen matrix and prints it out in a numpy
   * format along with the debug message of the file, function and line number
   * the print out is called at.
   *
  ...
  for (int i = 0; i < mat.rows() - 1; ++i) {
    if (i != 0)
      ...
      std::cout << "[ ";
    for (int j = 0; j < mat.cols() - 1; ++j) { std::cout << mat(i, j) << ", "; }
...
template std::vector<int> cpp_utils::arange<int>(int start, int stop, int step);
template std::vector<float> cpp_utils::arange<float>(float start, float stop,
                                                     float step);
template std::vector<double> cpp_utils::arange<double>(double start,
                                                       double stop,
                                                       double step);
...
std::vector<std::string> cpp_utils::get_files_in_directory(
    const std::string &directory, const std::string &ext = "") {
  ...
  if (ext.size() == 0) {
    ...
    for (auto &p : std::experimental::filesystem::recursive_directory_iterator(
             directory)) {
      if (p.path().extension() == ext) { files.push_back(p.path().string()); }
    }
  }
